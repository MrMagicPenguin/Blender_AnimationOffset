...
def RotateUV(obj, angle, pivot):
    # for uvIndex in range(len(uvMap.data)):
    #    uvMap.data[uvIndex].uv = Rotate2D(uvMap.data[uvIndex].uv, angle, pivot)
    # ? don't require the for loop here, this is handled by the foreach_get/set method

    uvs = np.empty(2 * len(obj.data.loops))
    uvs = obj.data.uv_layers.active.data.foreach_get("uv", uvs)
